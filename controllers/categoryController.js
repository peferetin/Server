...
const getCategories = async (req, res) => {
    ...
    try {
        ...
        res.set('Content-Range', `categories 0-${categories.length}/${categoriesCount}`)
...
const updateCategory = async (req, res) => {
    ...
    const { name, description } = req.body
    try {
        const category = await CategoryModel.findByIdAndUpdate(categoryID, { name, description }, { new: true })
        if (!category) {
            return res.status(404).json({ error: 'Category not found' })
        }
        return res.status(200).json(category)
...
export { getCategories, getCategoryById, getCategoryByName, createCategory, createCategories, deleteAllCategories, updateCategory }